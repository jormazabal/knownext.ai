...
- `filesystem_service`: local project folder scanning and file operations for folders, Markdown documents, supported images, and supported support files in the navigation tree.
- `config_service`: application configuration, including persisted layout widths, per-project open tabs and document-tree folder state, appearance preferences such as locale, zoom, theme mode, and primary accent color, diagnostics settings, AI provider/model settings, RAG settings, image-generation settings, and agentic task permission/limit settings. Per-task reasoning depth is carried on AI interaction requests, not stored as the primary app-level control.
...
- `config.json`: user-level application configuration such as sidebar and history panel widths, appearance settings including theme mode and primary accent color, diagnostics settings, AI model/RAG/agentic-task limits, plus the open document tabs per project, active document id, tab order, and per-project document-tree folder open paths.
...
Support files are also normal files inside the project folder. `filesystem_service` exposes supported non-Markdown, non-image files as `attachment` tree nodes for search, rename, move, delete, external-change review, backup, and Git/version flows. They do not have an internal viewer. Files with extractable text can be converted into backend-owned AI prompt context through `ai_context_service`; archive or binary formats that cannot be read remain manageable files but cannot become prompt text.
...
- Project support-file context sources are resolved by FastAPI from the current project path, extracted into temporary prompt-context storage, and sent as text context only when extraction succeeds. Unsupported support files must return a visible warning/error instead of being silently ignored.
