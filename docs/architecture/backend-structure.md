...
- `document_preview_service`: read-only reference document preview orchestration for project PDF, DOCX, and XLSX files. It validates project-relative paths, computes source fingerprints, creates stable preview contracts, serves PDF/text/workbook renditions, detects stale previews, and never modifies the source file.
- `export_service`: Markdown document export to `.md`, `.pdf`, and `.docx`. It receives the current Markdown buffer through the document export API, resolves project-relative image references, parses the same Markdown structures exposed by the editor, and writes to the user-selected output path in Tauri or returns generated bytes for browser development saves.
- `preview_cache_service`: preview rendition cache rooted in app data under `preview-cache/`, outside project folders.
- `office_conversion_service`: controlled LibreOffice headless conversion for DOCX paginated renditions. Missing LibreOffice falls back to the local DOCX basic renderer when possible; XLSX previews use the workbook grid and do not require Office conversion.
- `spreadsheet_preview_service`: read-only XLSX workbook extraction for sheet summaries and grid cells.
- `preview_text_service`: best-effort local text extraction for preview search and future FastAPI-mediated AI context.
- `export_template_service`: basic PDF/DOCX export template persistence. It stores a simple ASCII JSON file with page, typography, heading, spacing, link, and separator settings.
...
- `export-template-basic.json`: ASCII JSON export template used by PDF and DOCX generation. The app exposes common controls for it, and advanced users can edit the file directly.
...
- `preview-cache/<project-id>/<source-hash>/`: generated reference-document preview renditions such as `preview.json`, `document.pdf`, `text.json`, and `workbook.json`. Cache entries are invalidated by source fingerprint and preview engine version.
...
Support files are also normal files inside the project folder. `filesystem_service` exposes supported non-Markdown, non-image files as `attachment` tree nodes for search, rename, move, delete, external-change review, backup, and Git/version flows. PDF, DOCX, and XLSX attachments can be opened as read-only reference-document previews through `document_preview_service`; other support files remain manageable files without an internal viewer. Files with extractable text can be converted into backend-owned AI prompt context through `ai_context_service`; archive or binary formats that cannot be read remain manageable files but cannot become prompt text.
...
- `POST /api/documents/{document_id}/export` exports the Markdown buffer supplied by React, or the current backend document content when no buffer is supplied, to the user-selected `.md`, `.pdf`, or `.docx` path.
- `POST /api/documents/{document_id}/export/content` returns the generated export bytes for browser development flows where the frontend owns the save target instead of receiving a filesystem path from Tauri.
- `GET /api/config/export-template`, `PUT /api/config/export-template`, `POST /api/config/export-template/reset`, and `GET /api/config/export-template/path` expose the basic export template and its editable ASCII file path.
