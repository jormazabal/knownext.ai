...
- The document tree goes through `src/lib/api/projects.ts` and reflects folders, Markdown files, supported image assets, and supported support files under the active project's local folder. Backend trees are returned collapsed by default; React reapplies the user's persisted per-project folder open state after every tree refresh.
...
- Support-file import also goes through `src/lib/api/projects.ts`. React may display, search, filter, rename, move, delete, copy the project path, open supported PDF/DOCX/XLSX files as read-only reference-document tabs through `src/lib/api/documentPreviews.ts`, or request AI context for those files, but it does not read their bytes directly.
- Reference document previews live under `src/features/documentPreview`. The feature renders backend-prepared PDF/text/workbook renditions, uses PDF.js for paginated PDF and DOCX surfaces, shows XLSX as a read-only sheet grid, and keeps all loading, validation, conversion, cache, and external-open operations mediated by FastAPI.
