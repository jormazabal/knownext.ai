...
const apiBaseUrl = explicitApiBaseUrl || (process.env.KNOWNEXT_ANDROID_ALLOW_LOCAL_ENDPOINT === "1" ? `http://${resolveLanAddress()}:${port}` : "");
...
console.log(`ABI: ${abi}`);
console.log(`API: ${apiBaseUrl || "(not embedded; Android will ask on startup)"}`);
